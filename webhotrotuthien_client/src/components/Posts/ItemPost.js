...
function ItemPost({ onPostUpdate, post, xuLyThichBaiViet }) {
    ...
    useEffect(() => {
        const handleLike = () => {
            if (user !== null && post.likePost.map((element) => element.username).includes(user.username)) {
                setLike(true);
            } else {
                setLike(false);
            }
        ...
        const handleAuctioned = () => {
            if (user !== null && listAuction.map((element) => element.username).includes(user.username)) {
                setAuctioned(true);
            } else {
                setAuctioned(false);
            }
    ...
    const handleLikeClick = () => {
        ...
        sendMessage(
            JSON.stringify({
                ...
                content: `${user.username} vừa ${!like ? 'thích' : 'bỏ thích'} bài viết ${post.title} của bạn`,
            })
        );
        ...
        try {
            ...
            if (response.status === 201) {
                ...
                sendMessage(
                    JSON.stringify({
                        to: post?.user.username,
                        content: `${user.username} vừa đấu giá bài viết ${post.title} của bạn với số tiền là ${formatCurrency(formPrice.price) }`,
                    })
                );
    ...
    const handlSubmitComment = async (e) => {
        ...
        if (!user) {
            ...
            try {
                const response = await authApi().post(`${endpoints['comment']}${post.id}/`, formComment);
                if (response.status === 201) {
                    setFormComment({ content: '' });
                    sendMessage(
                        JSON.stringify({
                            to: post?.user.username,
                            content: `${user.username} vừa bình luận bài viết ${post.title} của bạn`,
                        })
                    );
                    onPostUpdate();
                } else {
                    ...
                    {user !== null && post.auctionStatus.id === 2 ? (
                        auctioned === false ? (
                            <div className="my-[18px]">
                                <Card.Text>
                                    <span className="font-bold mr-[8px]">Ngày bắt đầu đấu giá:</span>
                                    {post.startAuctionTime || 'Chủ bài viết không đề cập tới ngày bắt đầu'}
                                </Card.Text>
                                <Card.Text>
                                    <span className="font-bold mr-[8px]">Ngày kết thúc đấu giá: </span>
                                ...
                                {post.listComment.map((item, id) => {
                                    return (
                                        <li key={id} className="comment-item">
                                            <img width={100} src={item.image} className="comment-avt" alt="" />
                                            <div>
                                                <Link
